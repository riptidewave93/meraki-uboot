...
static int nand_read_page(struct mtd_info *mtd, int block, int page, uchar *dst)
{
	 ...
	 * in SDRAM
	 */
#if !defined(CONFIG_NAND_SPL_WITH_DDR_CALIBRATION)
	ecc_calc = (u_char *)(CONFIG_SYS_SDRAM_BASE + 0x10000);
#else
	ecc_calc = (u_char *)(CONFIG_SYS_SRAM_BASE);
#endif
		     ...
		     unsigned int uboot_size, uchar *dst)
{
	...
	/*
	 * Since NAND is treated like a stream of blocks, update the
	 * desired block offset to skip past any bad blocks which
	 * occur prior to the requested block offset.
	 */

	while (skip_bad < block) {
		if (nand_is_bad_block(mtd, skip_bad)) {
			block++;
			lastblock++;
		}
		skip_bad++;
	...
	while (block <= lastblock) {
		if (!nand_is_bad_block(mtd, block)) {
			...
			while (page < CONFIG_SYS_NAND_PAGE_COUNT) {
				...
				size_read += CONFIG_SYS_NAND_PAGE_SIZE;
				/* Check to see whether we should stop */
				if (size_read >= uboot_size)
					break;
				page++;
			}
...
#if defined(CONFIG_NAND_SPL_WITH_DDR_CALIBRATION)
	ret = nand_load(&nand_info, 4 << 10, 12 << 10,
                        (uchar *)(0xE3004000));
                        //(uchar *)(TEXT_BASE + CONFIG_SYS_NAND_BOOT_SPL_SIZE));
