...
int board_gpio_config(void)
{
    ...
    /* LEDs are active low, we want only orange, so prepare to drive
     * GPIO0-4,6 high */
    ar7100_reg_wr(AR7100_GPIO_OUT, 0x0000005f);
    /* Now enable GPIO0-6, 8, 10, 11 as outputs */
    ar7100_reg_wr(AR7100_GPIO_OE,  0x00000d7f);
...
#define GPIO_SIMPLE_LED_0 (1<<0)
#define GPIO_RESET  (1<<7)
...
int check_reset_button(void)
{
        ...
        udelay(200000);
#endif
        ar7100_reg_rmw_set(AR7100_GPIO_CLEAR, GPIO_SIMPLE_LED_0);
...
int checkboard (void)
{
    ...
    printf("AP96 (ar7100) U-boot " ATH_AP96_UBOOT_VERSION " MERAKI \n");
    return 0;
}
