...
static int bootm_start(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	mem_start += gd->ram_phybase;
#endif
	lmb_add(&images.lmb, mem_start, mem_size);
...
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	lmb_reserve(&images.lmb, lmb_pa((void *) images.os.load), 
			(load_end - images.os.load));
...
int do_bootd (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	char bootstring[20];
	if (getenv("AMP") != NULL)
		sprintf(bootstring, "bootcmd%d", CPUID);
	else
		sprintf(bootstring, "bootcmd");
	
	if (run_command (getenv (bootstring), flag) < 0)
		rcode = 1;
#else
