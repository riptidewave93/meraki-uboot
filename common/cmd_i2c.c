...
u32 i2c_eeprom_speed = CONFIG_SYS_I2C_SPEED;
u32 i2c_eeprom_addr = CONFIG_SYS_DEF_EEPROM_ADDR;
u32 i2c_eeprom_addr_len = CONFIG_SYS_I2C_EEPROM_ADDR_LEN;
...
/* i2c read/write from/to memory block
 *
 * Syntax:
 *	i2c mbr {i2c_chip} {addr}{.0, .1, .2} {memory address} {length}
 *	i2c mbw {i2c_chip} {addr}{.0, .1, .2} {memory address} {length}
 */
int do_i2c_mb ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	uchar	chip;
	ulong	addr;
	uint	alen;
	...
	/*
	 * Memory Address is always specified.
	 */
	 ...
	 * Bytes to read/write
	 */
	length = simple_strtoul(argv[4], NULL, 0);

	if (flag) {
		if (i2c_write(chip, addr, alen, (uchar *)mem, length) != 0) {
			puts ("Error writing the chip.\n");
		}
	} else {
		if (i2c_read(chip, addr, alen, (uchar *)mem, length) != 0) {
			puts ("Error reading the chip,\n");
		}
	}
...
int do_i2c(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (!strncmp(argv[0], "mbr", 3))
		return do_i2c_mb(cmdtp, 0, argc, argv);
	if (!strncmp(argv[0], "mbw", 3))
		return do_i2c_mb(cmdtp, 1, argc, argv);
...
U_BOOT_CMD(
	...
	"i2c mbr chip address[.0, .1, .2] address count - read from I2C device"
							" to memory address\n"
	"i2c mbw chip address[.0, .1, .2] adresss count - write to I2C device"
							" from memory address\n"
