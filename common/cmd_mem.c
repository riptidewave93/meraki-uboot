...
void cp_by_size(ulong target, ulong source, ulong len, int size)
{
	ulong s = source;
	ulong t = target;
	ulong count = len;
...
int do_cp_cksum_go ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	struct checksum_data cksum;
	unsigned long len = 0;
	...
	*/
	...
	addrs[0] = simple_strtoul(argv[1], NULL, 16);
	addrs[0] += base_address;

	addrs[1] = simple_strtoul(argv[2], NULL, 16);
	addrs[1] += base_address;
	...
	/* check if we are copying to Flash */
	if (addr2info(dest) != NULL) {
		puts ("Target address must be in memory.\n");
		return 0;
	}
	/* check if we are copying from memory */
	if ((addr2info(addrs[0]) == NULL) || (addr2info(addrs[1]) == NULL)) {
		puts ("Source address must be in Flash.\n");
		return 0;
	}
#else /* no Flash */
	puts ("No Flash.\n");
	...
	for (partno = 0; partno < 2; partno++) {
		addr = addrs[partno];
		valid = 1;

		if (addr2info(addr + sizeof(struct checksum_data)) == NULL) {
			printf ("part%d: invalid location - overflow on checksum header length 0x%08x\n",
				partno, sizeof(struct checksum_data));
			return 1;
		}

		cksum = *(struct checksum_data *)addr;
		magic = ntohl(cksum.magic);
		hlen = ntohl(cksum.header_length);
		len = ntohl(cksum.data_length);
#define MAX_LEN (16*1024*1024)
		...
		if (valid && (hlen < sizeof(struct checksum_data) || hlen > MAX_LEN || len > MAX_LEN || (addr2info(addr + hlen + len) == NULL))) {
		...
		cp_by_size(dest, addr + hlen, len, size);
		printf("done.\n");
		did_copy = 1;

		printf("part%d: Checking sha1 (from 0x%08x length %u) ... ", partno, dest, (unsigned int)len);
		sha1(sha1_res, (unsigned char *)dest, (unsigned int)len);
		r = memcmp(cksum.data_sha1, sha1_res, 20);
		printf("%s", r == 0 ? "match\n" : "mismatch\n");
	...
	if (!valid) {
		printf("!!!Could not find a valid partition!!!\n");
		if (did_copy)
			printf("Trying to boot part%d anyways\n", partno);
		else {
			printf("and could not even copy anything - giving up\n");
			return 1;
		}
	}
	...
	rc = ((ulong (*)(int, char *[]))dest) (1, &argv[3]);
	if (rc != 0) rcode = 1;
...
U_BOOT_CMD(
	cpckgo,    4,    1,    do_cp_cksum_go,
	"cpckgo  - copy, checksum, and go\n",
	"[.b, .w, .l] part1 part2 target\n    - copy part1 and do checksum, copy part 2 if checksum fails, then boot\n"
);
