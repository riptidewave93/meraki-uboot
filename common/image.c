		  ...
		  ulong *initrd_start, ulong *initrd_end)
{
	...
	lmb_reserve(lmb, lmb_pa(logbuffer_base() - LOGBUFF_OVERHEAD), 
			LOGBUFF_RESERVE);
#endif
	...
	if (rd_data) {
		if (!initrd_copy_to_ram) {	/* zero-copy ramdisk support */
			...
			*initrd_end = rd_data + rd_len;
			lmb_reserve(lmb, lmb_pa((void *) rd_data), rd_len);
		} else {
			phys_addr_t phy;
			if (initrd_high)
				phy = lmb_alloc_base(lmb, rd_len, 0x1000,
						lmb_pa((void *) initrd_high));
			else
				phy = lmb_alloc(lmb, rd_len, 0x1000);
			if (phy != 0)							
				*initrd_start = (ulong) lmb_va(phy);
			else
				*initrd_start = 0;
		...
		char **of_flat_tree, ulong *of_size)
{
	...
	if (relocate) {
		...
		of_start = (ulong) lmb_va(lmb_alloc_base(lmb, of_len, 0x1000,
					lmb_pa((void *) (CONFIG_SYS_BOOTMAPSZ + 
						bootmap_base))));
		if (of_start == 0) {
		...
		*of_flat_tree = fdt_blob;
		...
		lmb_reserve(lmb, lmb_pa(fdt_blob), of_len);
			...
			ulong bootmap_base)
{
	...
	cmdline = (char *)lmb_va(lmb_alloc_base(lmb, CONFIG_SYS_BARGSIZE, 0xf,
				 lmb_pa((void *)(CONFIG_SYS_BOOTMAPSZ + 
				 		bootmap_base))));
...
{
	*kbd = (bd_t *) lmb_va(lmb_alloc_base(lmb, sizeof(bd_t), 0xf,
				lmb_pa((void *) (CONFIG_SYS_BOOTMAPSZ + 
					bootmap_base))));
	if (*kbd == NULL)
		return -1;
