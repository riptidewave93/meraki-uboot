...
static int do_spi_flash_probe(int argc, char *argv[])
{
	...
	if (flash) {
		printf("SF: %d:%d probed\n", bus, cs);
		goto success;
	}

	new = spi_flash_probe(bus, cs, speed, mode);
	if (!new) {
		printf("SF: Failed to initialize flash at %u:%u\n", bus, cs);
		return 1;
	}
	...
	printf("SF: %u KiB %s at %u:%u is now current device\n",
...
static int do_spi_flash_read_write(int argc, char *argv[])
{
	...
	if (!buf) {
		puts("SF: Failed to map physical memory\n");
		return 1;
	}
	...
	if (ret) {
		printf("SF: flash %s failed\n", argv[0]);
		return 1;
	} else {
		printf("SF: flash %s success (%u bytes @ 0x%x)\n", 
					argv[0], (u32)len, (u32)offset);
	}
...
static int do_spi_flash(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (!flash) {
		puts("SF: No flash selected. Please run `sf probe'\n");
		return 1;
	}
