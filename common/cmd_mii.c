...
#include <miiphy.h>
#ifdef CONFIG_MERAKI
extern u32 athrs17_reg_read(unsigned int reg_addr);
extern void athrs17_reg_write(unsigned int reg_addr, unsigned int reg_val);
extern int apm_eth_miiphy_read(char *devname, unsigned char addr, unsigned char reg,
			unsigned short *value);
extern int apm_eth_miiphy_write(char *devname, unsigned char addr, unsigned char reg,
			 unsigned short value);	
#endif			 	
...
#ifdef CONFIG_MERAKI
extern int sw_current_dev(void);
extern void sw_select_dev(int sw_dev);
/* ---------------------------------------------------------------- */
int do_switch (cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	
	unsigned short	reg_addr = 0;
	unsigned int	reg_data = 0, i, error = 0, counter;	
	char *cmd;
  ...
  cmd = argv[1];
	...
	if(argv[2]!=NULL){			
		reg_addr = (unsigned short)simple_strtoul(argv[2], NULL, 16);
	}		
	
	if((argv[3]!=NULL)&&(strcmp(cmd, "write")==0)){			
		reg_data = (unsigned int)simple_strtoul(argv[3], NULL, 16);
	}		
	
	if (strcmp(cmd, "read") == 0) {		
		reg_data = athrs17_reg_read(reg_addr);					
		printf("Read From Switch Register: Address = 0x%x ,Reg Data = %04X\n",reg_addr, reg_data);	
   }
   
   if (strcmp(cmd, "write") == 0) {		
		athrs17_reg_write(reg_addr,reg_data);
		printf("Write to Switch register: Address = 0x%x ,Reg Data = %04X\n",reg_addr, reg_data);	
   }
  
   if (strcmp(cmd, "dev") == 0) {	
	   int sw_dev = 0;
	   if(argv[2]!=NULL){	
		   sw_dev = (unsigned short)simple_strtoul(argv[2], NULL, 10);
		   if ((sw_dev > 1) || (sw_dev < 0)) {
			   printf("Please select: 0-SW0, 1-SW1\n");		
		   }else {
			   sw_select_dev(sw_dev);
			   printf("Current dev: MDIO is connected to %s\n", (sw_dev==0)?"SW0":
																(sw_dev==1)?"SW1":"UNKNOWN");
		   }
	   }else {
		   sw_dev = sw_current_dev();
		   printf("Current dev: MDIO is connected to %s\n", (sw_dev==0)?"SW0":
															(sw_dev==1)?"SW1":"UNKNOWN");
	   }		
   }
   ...
   if (strcmp(cmd, "test") == 0) {	
	   if(argv[2]!=NULL){			
		   counter = (unsigned short)simple_strtoul(argv[2], NULL, 10);
	   }else {
		   counter = 20;
	   }		

	   printf("Loop 'write 0x624 value 0x007f7f7f and read' %d times\n", counter);
	   for(i=0; i<counter; i++){
		   athrs17_reg_write(0x624, 0x007f7f7f);
		   reg_data = athrs17_reg_read(0x624);		

		   printf("Switch Reg 0x%x = 0x%x\n",0x624,reg_data);	
		   if (reg_data != 0x007f7f7f)
			   error = 1;
	   }
	   if (!error) {
		   printf("PASS\n");	
   ...
   if (strcmp(cmd, "testrd") == 0) {	
	   ...
	   printf("Loop 'read 0x624' %d times\n", counter);
	   for(i=0; i<counter; i++){
		   reg_data = athrs17_reg_read(0x624);		
...
{
	unsigned char	phy_addr =0, reg_addr =0;
	unsigned short	reg_data, reg_data1, i, error, counter;		
#ifdef CONFIG_MII		
	char *devname;
#endif	
  char *cmd;
  ...
  cmd = argv[1];
	...
	if(argv[2]!=NULL){			
		phy_addr = (unsigned short)simple_strtoul(argv[2], NULL, 16);
	}		
	if(argv[3]!=NULL){			
		reg_addr = (unsigned short)simple_strtoul(argv[3], NULL, 16);
	}		
	
	if((argv[4]!=NULL)&&(strcmp(cmd, "write")==0)){			
		reg_data = (unsigned int)simple_strtoul(argv[4], NULL, 16);
	}		
	
	if (strcmp(cmd, "read") == 0) {						
		apm_eth_miiphy_read (devname,  phy_addr,reg_addr, &reg_data);		
		printf("Read From Switch Internal PHY Register:PHY Address = 0x%x Reg Address = 0x%x ,Reg Data = %04X\n",phy_addr,reg_addr, (reg_data&0xffff));	
   }
   
   if (strcmp(cmd, "write") == 0) {				
		apm_eth_miiphy_write(devname, phy_addr,reg_addr, reg_data);	
		printf("Write to Switch Internal PHY register:PHY Address = 0x%x Reg Address = 0x%x ,Reg Data = %04X\n",phy_addr,reg_addr, reg_data);	
   }
   ...
   if (strcmp(cmd, "test") == 0) {	
	   ...
	   printf("Loop 'write 0x0 value 0x0140 and read' %d times\n", counter);
	   for(i=0; i<counter; i++){
		   apm_eth_miiphy_write(devname, 0,0, 0x0140);	
		   apm_eth_miiphy_read(devname, 0,0, &reg_data);		
		   apm_eth_miiphy_read(devname, 0,2, &reg_data1);		

		   printf("PHY Addr = 0x%x Reg0 = 0x%x Reg2 = 0x%x\n",phy_addr,reg_data, reg_data1);	
		   if ((reg_data != 0x0140)||(reg_data1 != 0x004D))
			   error = 1;
	   }
	   if (!error) {
		   printf("PASS\n");	
   ...
   if (strcmp(cmd, "testrd") == 0) {	
	   ...
	   printf("Loop 'read 0x2, 0x3' %d times\n", counter);
	   for(i=0; i<counter; i++){	
		   apm_eth_miiphy_read(devname, 0,2, &reg_data);		
		   apm_eth_miiphy_read(devname, 0,3, &reg_data1);		

		   printf("PHY Addr = 0x%x Reg2 = 0x%x Reg3 = 0x%x\n",phy_addr,reg_data, reg_data1);	
		   if ((reg_data != 0x004D)||(reg_data1 != 0xD033))
			   error = 1;
	   }
	   if (!error) {
		   printf("PASS\n");	
   ...
   if (strcmp(cmd, "testsw") == 0) {	
	   apm_eth_miiphy_write(devname, 0x18,0, 0x0003);	
	   apm_eth_miiphy_read (devname, 0x10,0x12, &reg_data);		
	   apm_eth_miiphy_read (devname, 0x10,0x13, &reg_data1);		

	   printf("Read switch reg 0x624 by each mii command 0x%04x%04x\n",reg_data1, reg_data);	
   }
	return 0;
}
U_BOOT_CMD(
	switch,	4,	2,	do_switch,
	"Atheros Switch utility commands",			
	"\nswitch read <reg_addr> 				- Read  Switch Register \n"
	"switch write <reg_addr> <reg_data> 	- Write Switch Register \n"
	"switch dev <phy_num> 					- list or set current device \n"	
);

U_BOOT_CMD(
	phy,	5,	2,	do_phy,
	"Atheros Switch Internal PHY utility commands",			
	"\nphy read <phy_addr> <reg_addr> - Read  Switch Internal PHY Register \n"
	"phy write <phy_addr> <reg_addr> <reg_data> - Write Switch Internal PHY Register \n"
