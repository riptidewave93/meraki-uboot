...
long lmb_add(struct lmb *lmb, phys_addr_t base, phys_size_t size)
{
	...
	temp = lmb_add_region(_rgn, base, size);
#ifdef DEBUG
	lmb_dump_all(lmb);
#endif		
	return temp;
...
{	
	...
	temp = lmb_alloc_base(lmb, size, align, LMB_ALLOC_ANYWHERE);
#ifdef DEBUG
	lmb_dump_all(lmb);
#endif	
	return temp;	
...
void *__lmb_va(phys_addr_t addr)
{
	return (void *) (unsigned long) addr;
}
void *lmb_va(phys_addr_t) __attribute__((weak, alias("__lmb_va")));

phys_addr_t __lmb_pa(void *addr)
{
	return (phys_addr_t) (unsigned long) addr;
}
phys_addr_t lmb_pa(void *) __attribute__((weak, alias("__lmb_pa")));
