...
#define TIMEOUT_TFTP_BOOT_AUTO          3000
#define TIMEOUTCOUNT_TFTP_BOOT_AUTO     0
// WNC
...
// WNC Steve 2013/05/23
uchar isTftpComplete = 0;
// WNC

// WNC Steve 2013/05/21
extern uchar isTftpAutoBoot;
// WNC
...
// WNC
	...
	if (TftpRemoteIP) {
		printf("server %pI4", &TftpRemoteIP);
	} else {
		printf("broadcast server");
	}
	printf(" our IP address is %pI4", &NetOurIP);

	/* Check if we need to send across this subnet */
	if (TftpRemoteIP && NetOurGatewayIP && NetOurSubnetMask) {
...
// WNC Steve 2013/05/21
	if (isTftpAutoBoot)
	{
	    TftpTimeoutCountMax = TIMEOUTCOUNT_TFTP_BOOT_AUTO;
	    TftpTimeoutMSecs = TIMEOUT_TFTP_BOOT_AUTO;
	}
// WNC
	NetSetTimeout(TftpTimeoutMSecs, TftpTimeout);
