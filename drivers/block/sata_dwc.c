...
static int sata_dwc_start_dev; // This hold the start SATA DWC device number
...
int sata_dwc_init(int dev)
{
	...
	char *sata_controller;
	int chan = 0;
	int rc;
	int i;
	
	sata_dwc_start_dev = dev;

	phost = &host;

	// Read the environment variable sata_controller
	sata_controller = getenv("sata_controller");

#if defined(SATA1_BASE_ADDR)
	// Set the base address depends to use either SATA1 or SATA0
	if ( strcmp(sata_controller, "sata1") == 0 ) {
		printf("Do initialization on SATA1 ...\n");
		base = (u8*)SATA1_BASE_ADDR;
	}
	else 
#endif
	{
		printf("Do initialization on SATA0 as default\n");
	...
	if (status == 0x7f) {
		printf("sata_dwc: Hard Disk not found. Status = 0x%x\n", status);
...
static int sata_dwc_softreset(struct ata_port *ap)
{
	...
	if ((nsect == 0x55) && (lbal == 0xaa)) {
		printf("sata_dwc: Device found\n");
	} else {
		printf("sata_dwc: No device found\n");
		dev_state = SATA_NODEVICE;
		return FALSE;
	}
	...
	while (1) {
		...
		printf("sata_dwc: Hard Disk status is BUSY.\n");
		msleep(50);
	}
...
int sata_dwc_scan(int dev)
{
	...
	printf("sata_dwc: Waiting for device...");
	i = 0;
	while (1) {
		udelay(10000);
...
ulong sata_dwc_read(int device, ulong blknr, lbaint_t blkcnt, void *buffer)
{
...
ulong sata_dwc_write(int device, ulong blknr, lbaint_t blkcnt, const void *buffer)
{
	ulong start,blks, buf_addr;
	unsigned short smallblks;
	unsigned long datalen;
