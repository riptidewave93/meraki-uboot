...
#define BI_PHYMODE_TBI   5
    ...
    defined(CONFIG_405EX) || defined(CONFIG_APM82181)
...
static void ppc_4xx_eth_halt (struct eth_device *dev)
{
	...
	mtsdr(SDR0_ETH_CFG, val);
	 ...
	 /* don't bypass the TAHOE0 core for Linux */
        mfsdr(SDR0_ETH_CFG, val);
        val &= ~(SDR0_ETH_CFG_TAHOE0_BYPASS);
        mtsdr(SDR0_ETH_CFG, val);
#endif
...
#if defined(CONFIG_APM82181)
	/* bypass the TAHOE0 cores for U-Boot */
...
#endif  /* CONFIG_405EX || CONFIG_APM82181 */
...
#ifdef CONFIG_AR8327_SWITCH
static void ar8327_miiphy_write32(struct eth_device *dev, uint32_t reg_num, uint32_t value)
{
    ...
    /* change reg_addr to 16-bit word address, 32-bit aligned */
    reg_word_addr = (reg_num & 0xfffffffc) >> 1;

    /* configure register high address */
    phy_addr = 0x18;
    phy_reg = 0x0;
    phy_val = (uint16_t) ((reg_word_addr >> 8) & 0x1ff);  /* bit16-8 of reg address */
    miiphy_write(dev->name, phy_addr, phy_reg, phy_val);

    /* write register in higher address */
    phy_addr = 0x10 | ((reg_word_addr >> 5) & 0x7); /* bit7-5 of reg address */
    phy_reg = (uint8_t) (reg_word_addr & 0x1f);   /* bit4-0 of reg address */
    phy_val = (uint16_t) (value & 0xffff);
    miiphy_write(dev->name, phy_addr, phy_reg, phy_val);
    ...
    phy_addr = 0x10 | ((reg_word_addr >> 5) & 0x7); /* bit7-5 of reg address */
    phy_reg = (uint8_t) (reg_word_addr & 0x1f);   /* bit4-0 of reg address */
    phy_val = (uint16_t) (value >> 16 & 0xffff);
    miiphy_write(dev->name, phy_addr, phy_reg, phy_val);
}
...
static int ppc_4xx_eth_init (struct eth_device *dev, bd_t * bis)
{
...
#if defined(CONFIG_AR8327_SWITCH)
	//if the switch is transmitting, halt it before continuing
	if (!(in_be32((void *)EMAC_M0 + hw_p->hw_addr) & EMAC_M0_TXI))
		ppc_4xx_eth_halt(dev);
#endif
   ...
   !defined(CONFIG_440SPE) && !defined(CONFIG_APM82181)
	/* set RMII mode */
...
//Alex:20110317 - {
	{
#if defined(CONFIG_AR8327_SWITCH)
		/*      Config AR8327 MAC0 as RGMII mode by Atheros document(AR8327/AR8328 FAQ).
		 *
		 *       Address Offset  Value           Note            
		 *       ======================================================================
		 *       0x0             0x80000000      software reset
		 *       0x10            0x40000000      For AR8327 only
		 *       0x624           0x007f7f7f
		 *       0x4             0x07a00000      RGMII
		 *       0xc             0x01000000              
		 *       0x7c            0x7e
		 *       =======================================================================
		 */
		ar8327_miiphy_write32(dev, 0x0, 0x80000000);
		ar8327_miiphy_write32(dev, 0x10, 0x40000000);
		ar8327_miiphy_write32(dev, 0x624, 0x007f7f7f);
		ar8327_miiphy_write32(dev, 0x4, 0x07a00000);
		ar8327_miiphy_write32(dev, 0xc, 0x01000000);
		ar8327_miiphy_write32(dev, 0x7c, 0x7e);

		//set default led behavior
		ar8327_miiphy_write32(dev, 0x54, 0xcb35cb35);
		ar8327_miiphy_write32(dev, 0x58, 0xcf00cf00);

		//disable all the lan ports
		int i;
		for (i = 0; i < 4; i++) {
#define MII_PHY_CONTROL_REG        0x0
#define MII_PHY_CONTROL_POWER_DOWN (1 << 11)

			uint16_t reg_val;
			miiphy_read(dev->name, i, MII_PHY_CONTROL_REG, &reg_val);
			if (!(reg_val & MII_PHY_CONTROL_POWER_DOWN)) {
				printf("Disabling port %d\n", i);
				reg_val |= MII_PHY_CONTROL_POWER_DOWN;
				miiphy_write(dev->name, i, MII_PHY_CONTROL_REG, reg_val);
			}
		}
#endif
		goto get_speed;
...
#if defined(CONFIG_RTL8211CL_PHY)
	miiphy_write (dev->name, reg, 0x1f, 0x5);
	miiphy_write (dev->name, reg, 0xc, 0x0);
	miiphy_read(dev->name, reg, 0x1, &reg_short);
	reg_short |= 0x0080;
	miiphy_write (dev->name, reg, 0x1, reg_short);
	miiphy_write (dev->name, reg, 0x1f, 0x0);
	miiphy_write(dev->name, reg, 0x9,0x0700);

    miiphy_write(dev->name, reg, 31,0x0002);
    miiphy_write(dev->name, reg, 24,0xA1E0);
    miiphy_write(dev->name, reg, 31,0x0000);
#endif

#if defined(CONFIG_AR8035_PHY)
	miiphy_write (dev->name, reg, 0x1d, 0x5);
	miiphy_write (dev->name, reg, 0x1e, 0x2d47);
	miiphy_write (dev->name, reg, 0x1d, 0xb);
	miiphy_write (dev->name, reg, 0x1e, 0xbc20);	//tx clock delay, bit[6:5]"00":02.ns;"01":1.2ns;"10":2.1ns;"11"=3.0ns
#endif
		miiphy_reset (dev->name, reg);
