...
/*-----------------------------------------------------------------------
 */
static int flash_isset_D7 (flash_info_t * info, flash_sect_t sect,
			 uint offset, uchar cmd)
{
	void *addr;
//	cfiword_t cword;
...
//	flash_make_cmd (info, cmd, &cword);
	...
	case FLASH_CFI_8BIT:
		...
		break;
	case FLASH_CFI_16BIT:
		retval = (flash_read16(addr) & 0x8000) == 0x8000; // 0x8000 -> check QD7 = 1
		break;
	case FLASH_CFI_32BIT:
		retval = flash_read32(addr) != flash_read32(addr);
		break;
			       ...
			       ulong tout, char *prompt)
{
	...
	if(!strcmp(prompt, "erase")){
		start = get_timer (0);
		while (!flash_isset_D7(info, sector, 0, 0)){
