...
#define FLASH_CFI_STEP	1	/* 16 bit width */
#if (CONFIG_SYS_FLASH_CFI_WIDTH == FLASH_CFI_8BIT)
#undef FLASH_OFFSET_CFI
#define FLASH_OFFSET_CFI	0xAA
#undef FLASH_OFFSET_CFI_ALT
#define FLASH_OFFSET_CFI_ALT	0xAAA
#undef FLASH_OFFSET_CFI_RESP
#define FLASH_OFFSET_CFI_RESP           0x20
#undef FLASH_CFI_STEP
#define FLASH_CFI_STEP	2
#endif
...
static void cmdset_amd_read_jedec_ids(flash_info_t *info)
{
	...
	case FLASH_CFI_8BIT:
		info->device_id = flash_read_uchar (info,
						FLASH_OFFSET_DEVICE_ID*FLASH_CFI_STEP);
		if (info->device_id == 0x7E) {
		...
		unsigned int start, size_t len)
{
	...
	for (i = 0; i < len; i++)
		p[i] = flash_read_uchar(info, start + i*FLASH_CFI_STEP);
}
...
static int __flash_detect_cfi (flash_info_t * info, struct cfi_qry *qry)
{
	...
	for (cfi_offset=0;
		...
		if (flash_isequal (info, 0, FLASH_OFFSET_CFI_RESP, 'Q')
		    && flash_isequal (info, 0, FLASH_OFFSET_CFI_RESP + 1*FLASH_CFI_STEP, 'R')
		    && flash_isequal (info, 0, FLASH_OFFSET_CFI_RESP + 2*FLASH_CFI_STEP, 'Y')) {
...
#if defined(CONFIG_SECTOR_REDUCE) && defined(CONFIG_SIZE_REDUCE)
	info->sector_count = sect_cnt - CONFIG_SECTOR_REDUCE;
	info->size = (1 << qry.dev_size) - CONFIG_SIZE_REDUCE;
#else
		...
		if ((info->interface == FLASH_CFI_X8X16) &&
			...
			if (info->portwidth >=2) info->portwidth >>= 1;
		}
