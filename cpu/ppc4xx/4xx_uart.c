...
#define UART0_BASE	CONFIG_SYS_PERIPHERAL_BASE
#define UART1_BASE	(CONFIG_SYS_PERIPHERAL_BASE + 0x00001000)
#define UART2_BASE  	(CONFIG_SYS_PERIPHERAL_BASE + 0x00007000)
#define UART3_BASE  	(CONFIG_SYS_PERIPHERAL_BASE + 0x00008000)
#else
...
#error "External serial clock not supported on AMCC PPC405EP!"
	...
	#define UART_REG_ALIGNED	4 /* UART's are 4-bytes aligned */
#else
	#define UART_REG_ALIGNED	1
#endif

#define UART_RBR    (0x00*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_THR    (0x00*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_IER    (0x01*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_IIR    (0x02*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_FCR    (0x02*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_LCR    (0x03*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_MCR    (0x04*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_LSR    (0x05*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_MSR    (0x06*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_SCR    (0x07*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_DLL    (0x00*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
#define UART_DLM    (0x01*UART_REG_ALIGNED+UART_REG_ALIGNED-1)
...
int serial_init_dev(unsigned long base)
{
	...
	/* No udiv needed, always code 1 */
	udiv = 1;
	tmp  = gd->baudrate * 16;
#ifdef CONFIG_SYS_EXT_SERIAL_CLOCK
	sysinfo.freqUART = CONFIG_SYS_EXT_SERIAL_CLOCK;
	out_be32((void *)(CONFIG_SYS_CSR_BASE + 0x620cc),
		in_be32((void *)(CONFIG_SYS_CSR_BASE + 0x620cc)) | 0x20);
#endif
	bdiv = (sysinfo.freqUART + tmp / 2) / tmp;
...
int serial_getc_dev (unsigned long base)
{
	...
	return (0x000000ff & (int) in_8((u8 *)base + UART_RBR));
}
