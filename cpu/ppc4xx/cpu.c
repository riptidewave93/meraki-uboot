...
int checkcpu (void)
{
...
#endif
#if defined(CONFIG_440)
#if defined(CONFIG_460EX) || defined(CONFIG_460GT) || defined(CONFIG_APM86XXX)
	puts("60");
#else
	puts("40");
#endif
	...
	printf (" at %s MHz PLB=%lu MHz\n",
		strmhz(buf, clock),
		sys_info.freqPLB / 1000000);
	printf ("       (SOC=%lu AXI=%lu AHB=%lu APB=%lu EBC=%lu",
...
unsigned long get_tbclk (void)
{
	...
	get_sys_info(&sys_info);
#if defined(CONFIG_APM86XXX)
	return (sys_info.freqProcessor[mfspr(SPRN_PIR)]);
#else
...
#endif
#if defined (CONFIG_APM86xxx_ENET)
