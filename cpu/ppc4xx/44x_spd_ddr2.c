...
#define PPC4xx_IBM_DDR2_DUMP_DCR(mnemonic)                         \
        do {                                                            \
                u32 data;                                               \
...
/*Move from 1.3.2 include/asm-ppc/ppc4xx-sdram.h*/
#ifndef __ASSEMBLY__
...
static void check_voltage_type(unsigned long *dimm_populated,
			       unsigned char *iic0_dimm_addr,
			       unsigned long num_dimm_banks);
#if defined(SDRAM_R0BAS) /* MQ exists */
	...
	if (((mcopt2 & SDRAM_MCOPT2_DCEN_MASK) == SDRAM_MCOPT2_DCEN_ENABLE)
		...
		for (i = 0; i < MAXBXCF; i++) {
			...
			if ((mb0cf & SDRAM_BXCF_M_BE_MASK) == SDRAM_BXCF_M_BE_ENABLE) {
				...
				#else /* For SDRAM controller that does not have MQ */
...
phys_size_t initdram(int board_type)
{
	...
	/* program PLB slave options register */
	mtsdram(SDRAM_PLBOPT, 0xE0000000);
#endif
	/*------------------------------------------------------------------
	 ...
	 *-----------------------------------------------------------------*/
		...
		/* Set SDR0_MEMCLK_SEL register to select 90degree phase offset */
		mtsdr(SDR0_MEMCLK_SEL, MEMCLK_SEL_90DEG_OFF);
		/* Read back to check */
		mfsdr(SDR0_MEMCLK_SEL, val);
		debug("SDR0_MEMCLK_SEL set to 0x%08x\n", (unsigned int)val);
		#if 0	
		mfsdram(SDRAM_WRDTR, val);
		mtsdram(SDRAM_WRDTR, (val & ~(0x0000003F))| 0x00000020); /* Delay 90degree */
		#endif
	#else
...
#if 1
	/* We use 75ohm odt for 460sx */
	val |= (SDRAM_MEMODE_DIC_NORMAL | SDRAM_MEMODE_DLL_ENABLE
		| SDRAM_MEMODE_RTT_75OHM | SDRAM_MEMODE_DQS_ENABLE ); 
#else
	/* Set to 50OHM for DDR 800 */
	val |= (SDRAM_MEMODE_DIC_NORMAL | SDRAM_MEMODE_DLL_ENABLE
                | 0x00000040 | SDRAM_MEMODE_DQS_ENABLE );	/* 50 Ohm for DDR-800 */
#endif		
#if 0
	/* This is the previous method of calibrating DDR using
	 * additive latency, however this method prove to be ineffective.
	 * Now we use offset CAS latency method. This code may be 
	 * removed later.
	 */
	PPC4xx_SYS_INFO board_cfg;
	get_sys_info(&board_cfg);

	if (board_cfg.freqDDR > 200000100) {
		val |= JEDEC_MA_EMR_AL_DDR2_2_CYC;
	}
#endif
	mtsdram(SDRAM_MEMODE, val);
#else
	 ...
	 *-----------------------------------------------------------------*/
#if defined(CONFIG_PPC4xx_DDR_AUTOCALIBRATION)
#if defined(CFG_FIXED_WITH_LARGE_RAM)
	if (sdram_memsize() > (CONFIG_MAX_MEM_MAPPED)) {
		debug("Set fixed value\n");
		/* FIXME: configure Fixed value */
		mtsdram(SDRAM_WRDTR, CFG_WRDTR_FIXED);
        	mtsdram(SDRAM_CLKTR, CFG_CLKTR_FIXED);
	        mtsdram(SDRAM_RDCC, CFG_RDCC_FIXED);
	        mtsdram(SDRAM_RQDC, CFG_RQDC_FIXED);
	        mtsdram(SDRAM_RFDC, CFG_RFDC_FIXED);	
	} else
#endif
		DQS_autocalibration();
#else
	...
	set_mcsr(get_mcsr());
        ...
        puts("Perform memory testing\n");
        do_mem_mtest(NULL, 1, 1, NULL);
#endif
...
#if defined(CONFIG_460SX) || \
    defined(CONFIG_APM82181) || defined(CONFIG_405EX)
	val = board_cfg.freqDDR;
#else
	mfsdr(SDR0_DDR0, val);
	val = MULDIV64((board_cfg.freqPLB), SDR0_DDR0_DDRM_DECODE(val), 1);
#endif
	printf(" enabled, %d Mb/s", (val * 2) / 1000000);
			  ...
			  unsigned long num_dimm_banks)
{
...
#if !defined(CONFIG_460SX)	/* There are not RWOO and WOOO in SDRAM_MCOPT1 register of 460SX */
	mcopt1 |= SDRAM_MCOPT1_RWOO_DISABLED;
	mcopt1 |= SDRAM_MCOPT1_WOOO_DISABLED;
#endif
	mcopt1 |= SDRAM_MCOPT1_DCOO_DISABLED;
#if defined(CONFIG_APM82181)
	mcopt1 |= SDRAM_MCOPT1_DREF_DEFER_4; /* defer refresh upto 4 cycles */
#else
	...
	/* 
	 * FIXME: Most of current DIMMs are 64-bit, 
	 * the code does not care about 32-bit DIMMs 
         */
#if defined(CONFIG_APM82181) || defined(CONFIG_405EX)
	if (gd->sdram_16bit)
		mcopt1 |= SDRAM_MCOPT1_DMWD_16;
	else
			 ...
			 unsigned long num_dimm_banks)
{
	...
	g_total_dimm = total_dimm;
	
	debug("codt = 0x%08x\n", (unsigned int)codt);
	mtsdram(SDRAM_CODT, codt);
#ifdef CONFIG_DDR_CODT_FIXED
	/*
	 * In 460SX, the CODT and MODT is now fixed.
	 * The method for automatically determining the
	 * setting is unclear. It needs to be explained 
	 * by hw guys.
	 */
	mtsdram(SDRAM_CODT, CONFIG_DDR_CODT_FIXED);
#endif
	debug("modt0 = 0x%08x\n", (unsigned int)modt0);
...
#ifdef CONFIG_DDR_MODT0_FIXED
	mtsdram(SDRAM_MODT0, CONFIG_DDR_MODT0_FIXED);
#endif
#ifdef CONFIG_DDR_MODT1_FIXED
	mtsdram(SDRAM_MODT1, CONFIG_DDR_MODT1_FIXED);
#endif
#ifdef CONFIG_DDR_MODT2_FIXED
	mtsdram(SDRAM_MODT2, CONFIG_DDR_MODT2_FIXED);
#endif
#ifdef CONFIG_DDR_MODT3_FIXED
	mtsdram(SDRAM_MODT3, CONFIG_DDR_MODT3_FIXED);
#endif
}
			    ...
			    int write_recovery)
{
	...
	if ((dimm_populated[0] == SDRAM_DDR1) || (dimm_populated[1] == SDRAM_DDR1)) {
		...
		/* FIXME: This information is not exist in SPD 
		 * So choose a typical value 
		 */
		t_mrd_ns = 10;
		t_mrd_clk = MULDIV64(sdram_freq, t_mrd_ns, ONE_BILLION);
                ddr_check = MULDIV64(ONE_BILLION, t_mrd_clk, t_mrd_ns);
                if (sdram_freq != ddr_check)
                        t_mrd_clk++;

		/* Wait NOP - DDR2 standard: NOP is issued during  400ns */
		t_nop_clk = MULDIV64(sdram_freq, 400, ONE_BILLION);
		ddr_check = MULDIV64(ONE_BILLION, t_nop_clk, 400);
                if (sdram_freq != ddr_check)
                        t_nop_clk++;
		t_nop_clk /= 2;
		...
		mtsdram(SDRAM_INITPLR4, SDRAM_INITPLR_ENABLE | emr |
					SDRAM_INITPLR_IMWT_ENCODE(t_mrd_clk));
		/* MR w/ DLL reset - 200 cycle wait for DLL reset - DDR2 standard */
		...
		mtsdram(SDRAM_INITPLR12, SDRAM_INITPLR_ENABLE | OCD_CALIB_DEF |
					 SDRAM_INITPLR_IMWT_ENCODE(t_mrd_clk) | emr);
		/* EMR OCD Exit */
			 ...
			 int *write_recovery)
{
	...
	unsigned long cycle_6_0_clk;
	unsigned long cycle_7_0_clk;
	...
	unsigned long max_6_0_tcyc_ns_x_100;
	unsigned long max_7_0_tcyc_ns_x_100;
	...
	unsigned char cas_6_0_available;
	unsigned char cas_7_0_available;
#if !defined(CONFIG_460SX) && \
    !defined(CONFIG_APM82181) && !defined(CONFIG_405EX)
	unsigned long sdr_ddrpll;
#endif
	...
	for (dimm_num = 0; dimm_num < num_dimm_banks; dimm_num++) {
		...
		if (dimm_populated[dimm_num] != SDRAM_NONE) {
			...
			for (cas_index = 0; cas_index < 3; cas_index++) {
				...
				debug("cas_index=%d: cycle_time_ns_x_100=%d\n", (unsigned int)cas_index,
				      (unsigned int)cycle_time_ns_x_100[cas_index]);
			}
			...
			if (sdram_ddr1) {
				 ...
				 */
                                if (((cas_bit & 0x80) == 0x80) && (cas_index < 3) &&
                                    (cycle_time_ns_x_100[cas_index] != 0)) {
                                        max_7_0_tcyc_ns_x_100 = max(max_7_0_tcyc_ns_x_100,
                                                                    cycle_time_ns_x_100[cas_index]);
                                        cas_index++;
                                } else {
                                        if (cas_index != 0)
                                                cas_index++;
                                        cas_7_0_available = FALSE;
                                }

                                if (((cas_bit & 0x40) == 0x40) && (cas_index < 3) &&
                                    (cycle_time_ns_x_100[cas_index] != 0)) {
                                        max_6_0_tcyc_ns_x_100 = max(max_6_0_tcyc_ns_x_100,
                                                                    cycle_time_ns_x_100[cas_index]);
                                        cas_index++;
                                } else {
                                        if (cas_index != 0)
                                                cas_index++;
                                        cas_6_0_available = FALSE;
                                }
	...
	cycle_6_0_clk = MULDIV64(ONE_BILLION, 100, max_6_0_tcyc_ns_x_100) + 10;
	cycle_7_0_clk = MULDIV64(ONE_BILLION, 100, max_7_0_tcyc_ns_x_100) + 10;
	debug("cycle_3_0_clk=%d\n", (unsigned int)cycle_3_0_clk);
	debug("cycle_4_0_clk=%d\n", (unsigned int)cycle_4_0_clk);
	debug("cycle_5_0_clk=%d\n", (unsigned int)cycle_5_0_clk);
	debug("cycle_6_0_clk=%d\n", (unsigned int)cycle_7_0_clk);
	debug("cycle_7_0_clk=%d\n", (unsigned int)cycle_7_0_clk);
	...
	if (sdram_ddr1 == TRUE) { /* DDR1 */
		...
		debug("cas_6_0_available=%d\n", cas_5_0_available);
		debug("cas_7_0_available=%d\n", cas_7_0_available);
		...
		} else if ((cas_6_0_available == TRUE) && (sdram_freq <= cycle_6_0_clk)) {
#ifdef CONFIG_460SX
			/* This is offset method to obtain calibration window in 460SX
			 * at 800Mhz. This code needs to be examined further.
			 */
			mmode |= SDRAM_MMODE_DCL_DDR2_6_0_CLK;
#else
			mmode |= SDRAM_MMODE_DCL_DDR2_6_0_CLK;
//	*selected_cas = DDR_CAS_7;
#endif
			*selected_cas = DDR_CAS_6;
		} else if ((cas_7_0_available == TRUE) && (sdram_freq <= cycle_7_0_clk)) {
			mmode |= SDRAM_MMODE_DCL_DDR2_7_0_CLK;
			*selected_cas = DDR_CAS_7;
		} else {
			printf("ERROR: Cannot find a supported CAS latency with the installed DIMMs.\n");
			printf("Only DIMMs DDR2 with CAS latencies of 3.0, 4.0, and 5.0 are supported.\n");
			printf("Make sure the PLB speed is within the supported range of the DIMMs.\n");
			printf("cas3=%d cas4=%d cas5=%d cas6=%d cas7=%d\n",
			       cas_3_0_available, cas_4_0_available, cas_5_0_available, cas_6_0_available, cas_7_0_available);
			printf("sdram_freq=%lu cycle3=%lu cycle4=%lu cycle5=%lu cycle6=%lu cycle7=%lu\n\n",
			       sdram_freq, cycle_3_0_clk, cycle_4_0_clk, cycle_5_0_clk, cycle_6_0_clk, cycle_7_0_clk);
			spd_ddr_init_hang ();
		}
	}
		       ...
		       unsigned long num_dimm_banks)
{
	...
	/* default values */
	#if 1
	/* Clear LDOF bit to gain performance - dangerous :) */
		...
		if (dimm_populated[dimm_num] != SDRAM_NONE) {
			...
			for (ind_bank = 0; ind_bank < 2; ind_bank++) {
				...
				if (gd->sdram_16bit) {
					...
					break;
				case 0x08: /* Is there any 16MB option? */
				if (gd->sdram_16bit) {
			...
			#endif /* defined(CONFIG_APM82181) || defined(CONFIG_405EX) */	
			}
			...
			for (ind_rank = 0; ind_rank < num_ranks; ind_rank++) {
				...
				mtsdram(SDRAM_MB0CF +
					...
					mode | baseadd_size | SDRAM_MBXCF_BASE_ENCODE(rank_base_addr));
	...
	for (dimm_num = 0; dimm_num < num_dimm_banks; dimm_num++) {
		if (dimm_populated[dimm_num] != SDRAM_NONE) {
			...
			for (i = 0; i < num_ranks; i++)	{
				debug("Dimm#%d: Programming R%dBAS register with value 0x%08x\n", \
					(unsigned int)dimm_num, (unsigned int)(dimm_num*2 + i), \
					(unsigned int)(SDRAM_RXBAS_SDBA_ENCODE(rank_base_addr) | baseadd_size));
...
#else
	mtdcr(SDRAM_PLBADDUHB, 0xE0000000); /* MQ0_BAUH */
#endif
...
#else
#if 0
	mtdcr(SDRAM_CONF1HB,0x80008c80); /* CF1H config High bandwidth*/
        mtdcr(SDRAM_CONF1LL,0x80008c80); /* CF1L config low latency */
        /* config between Low latency and High bandwitdh paths */
        mtdcr(SDRAM_CONFPATHB,0x88a68000);
#else
	mtdcr(SDRAM_CONF1HB,0x00001c40); /* CF1H config High bandwidth*/
        mtdcr(SDRAM_CONF1LL,0x80001e00); /* CF1L config low latency */
        /* config between Low latency and High bandwitdh paths */
     	mtdcr(SDRAM_CONFPATHB,0x29800090);
#endif
			...
			unsigned long tlb_word2_i_value)
{
	...
	if ((dram_size = sdram_memsize()) > (CONFIG_MAX_MEM_MAPPED)) {
	...
	if ((mcopt1 & SDRAM_MCOPT1_MCHK_MASK) != SDRAM_MCOPT1_MCHK_NON) {
		...
		if (((mcopt2 & SDRAM_MCOPT2_DCEN_MASK) == SDRAM_MCOPT2_DCEN_ENABLE)
			...
			program_ecc_addr(0, dram_size, tlb_word2_i_value);
		}
	}
...
static int short_mem_test(void)
{
	...
	for (bxcr_num = 0; bxcr_num < MAXBXCF; bxcr_num++) {
		...
		if ((bxcf & SDRAM_BXCF_M_BE_MASK) == SDRAM_BXCF_M_BE_ENABLE) {
			...
			#else /* SDRAM controller does not have MQ */
				base_addr = (u32 *)((bxcf & 0xfff80000) << 3);
			#endif
...
static void test(void)
{
	...
	for (dimm_num = 0; dimm_num < MAXDIMMS; dimm_num++) {
		mfsdram(SDRAM_MB0CF + (dimm_num << 2), bxcf[dimm_num]);
		...
		if ((bxcf[dimm_num] & SDRAM_BXCF_M_BE_MASK) == SDRAM_BXCF_M_BE_ENABLE) {
			...
			#else /* SDRAM controller does not have MQ */
				membase = (u32 *)((bxcf[dimm_num] & 0xfff80000) << 3);
			#endif
			/*------------------------------------------------------------------
			 * Run the short memory test.
			 *-----------------------------------------------------------------*/
...
phys_size_t initdram(int board_type)
{
...
#if defined(CONFIG_440) && !defined(CONFIG_APM82181)
...
#ifdef CONFIG_460SX
	mtsdram(SDRAM_MODT2, CONFIG_SYS_SDRAM0_MODT2);
	mtsdram(SDRAM_MODT3, CONFIG_SYS_SDRAM0_MODT3);
#endif
	 ...
	 * while auto-calibrating and ECC generation
	 */
	/* Maui supports setting TLB entries */
...
#endif /* defined(CONFIG_440) && !defined(CONFIG_APM82181) */
...
inline void ppc4xx_ibm_ddr2_register_dump(void)
{
...
#if defined(CONFIG_460SX)
	{
		u32 val;
		mfsdr(SDR0_MEMCLK_SEL, val);
        	debug("%18s[4401] = 0x%08x\n", "SDR0_MEMCLK_SEL", val);
	}
#endif
#if (defined(CONFIG_440SP) || defined(CONFIG_440SPE) || \
     defined(CONFIG_460EX) || defined(CONFIG_460GT)|| defined(CONFIG_460SX) )
	PPC4xx_IBM_DDR2_DUMP_DCR(PLBADDULL);
	PPC4xx_IBM_DDR2_DUMP_DCR(PLBADDUHB);
	PPC4xx_IBM_DDR2_DUMP_DCR(CONF1HB);
	PPC4xx_IBM_DDR2_DUMP_DCR(CONF1LL);
	PPC4xx_IBM_DDR2_DUMP_DCR(CONFPATHB);
	PPC4xx_IBM_DDR2_DUMP_DCR(R0BAS);
	PPC4xx_IBM_DDR2_DUMP_DCR(R1BAS);
	PPC4xx_IBM_DDR2_DUMP_DCR(R2BAS);
	PPC4xx_IBM_DDR2_DUMP_DCR(R3BAS);
#endif /* (defined(CONFIG_440SP) || ... */
#if defined(CONFIG_405EX) || defined(CONFIG_APM82181)
