...
void ft_cpu_setup(void *blob, bd_t *bd)
{
	...
	do_fixup_by_prop_u32(blob, "device_type", "cpu", 4, "timebase-frequency",
			     sys_info.freqProcessor[mfspr(SPRN_PIR)], 1);
	do_fixup_by_prop_u32(blob, "device_type", "cpu", 4, "clock-frequency",
			     sys_info.freqProcessor[mfspr(SPRN_PIR)], 1);
	do_fixup_by_prop_u32(blob, "device_type", "cpu", 4, "plb-frequency",
			      sys_info.freqPLB, 1);
#else
	do_fixup_by_prop_u32(blob, "device_type", "cpu", 4, "timebase-frequency",
			     bd->bi_intfreq, 1);
...
#ifdef CONFIG_MP
	fdt_fixup_memory(blob, (u64)gd->ram_phybase + bd->bi_memstart, 
			(u64)bd->bi_memsize);
#else
	 ...
	 */
#ifdef CONFIG_APM86XXX
	off = fdt_path_offset(blob, "/hbf/ahb/apb");
#else
