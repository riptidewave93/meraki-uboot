...
/* Enable/disable NDFC command */
#define DISABLE_NDFC	0
#define ENABLE_NDFC	1
static int enable_ndfc(int cs)
{
	u32 sdr0_cust0;

	/* SDR Setting - enable NDFC */
        mfsdr(SDR0_CUST0, sdr0_cust0);
        sdr0_cust0 = ((sdr0_cust0 | SDR_CUST0_NDFC_ENABLE) &
                        (~SDR_CUST0_NDFC_BW_MASK)) |
                        SDR_CUST0_NDFC_BW_8_BIT |
                        SDR_CUST0_NCG(cs);
...
static int disable_ndfc(int cs)
{
	u32 sdr0_cust0;

	 /* SDR Setting - disable NDFC */
        mfsdr(SDR0_CUST0, sdr0_cust0);                
        sdr0_cust0 = (sdr0_cust0 & (~SDR_CUST0_NDFC_ENABLE)) &
			SDR_CUST0_NCG_EBC;
        mtsdr(SDR0_CUST0, sdr0_cust0);
...
static int do_ndfc(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
        u32 action;
	u32 cs;
	...
	cs = simple_strtoul(argv[2], NULL, 32);	
		...
		disable_ndfc(cs);
                ...
                printf("Unsupported option - only enable|disable support\n");
                return 1;
        }
...
U_BOOT_CMD(
        ndfc, 3,      0,      do_ndfc,
        "ndfc - enable|disable NDFC\n",
        "<enable|disable> <cs> - enable|disable NDFC with chip select cs \n"
        );
#endif /* defined(CONFIG_APM82181)*/
