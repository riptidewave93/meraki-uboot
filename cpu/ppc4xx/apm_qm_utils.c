...
void dump_qstate(struct mb_qm_qstate *qstate)
{
	QM_PRINT("\nQUEUE STATE DUMP\n");
...
int mb_qm_rx_msg_util(int qid, int mbid)
{
	struct mb_qm_msg_desc msg_desc;
...
int mb_qm_send_msg_util(int qid, int mbid)
{
	struct mb_qm_msg_desc msg_desc;
	struct mb_qm_msg32 msg;
	...
	mb_qm_push_msg(&msg_desc);

	return 0;
}

int mb_qm_dealloc_buf_util(int qid, int mbid)
{
	struct mb_qm_msg_desc msg_desc;
	...
	mb_qm_fp_dealloc_buf(&msg_desc);

	return 0;
}

int mb_qm_alloc_buf_util(int qid, int mbid)
{
	struct mb_qm_msg_desc msg_desc;
	struct mb_qm_msg16 msg;
