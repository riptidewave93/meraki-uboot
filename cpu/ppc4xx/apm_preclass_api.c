...
#ifdef CLE_SHADOW
static inline void apm_cle_shadow_init(void)
{
	if (!cle_shadow_done) {
		       ...
		       u8 set_type)
{
	...
	case EWDN:
		...
		ewdn->chain_compare = dn->chain_compare & DN_CHAIN_SIZE;
		PCLS_PRNT(" EW: Type %d "
		...
		for (i = 0; i < dn->num; i++) {
			...
			PCLS_PRNT("  %d: VBit %d Mask %04x Data %04x OTyp %d "
	...
	case FWDN:
		...
		fwdn->chain_compare = dn->chain_compare & DN_CHAIN_SIZE;
		PCLS_PRNT("FW%d: Type %d "
				"RPtr %d SBSt %d BySt %d HdSt %d Last %d CorC %d"
	...
	case KN:
		...
		PCLS_PRNT(" KN: Type %d Indx %d Prio %d RPtr %d\n",
				node->type, index,
...
int apm_ptree_node_config(u8 port, struct ptree_node *node)
{
	...
	if (node->type > EWDN) {
		...
		if (node->type == FWDN) {
			...
			/* check if its the same type of node */
				...
				if ((pns->config_mask >> node->location)
					& 1) {
	...
	} else if (node->type == EWDN) { /* 8-way node */
		memset(data, 0, sizeof(data));
