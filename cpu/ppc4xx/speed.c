 ...
 *            with latest EAS
 */
#if !defined(CONFIG_APM82181) /* APM82181 has different bootstrap register layout */
...
void get_sys_info (sys_info_t * sysInfo)
{
	...
	temp = clk_plld_fwdva(plld);
	sysInfo->pllFwdDivA = get_cpr0_fwdv(temp);

	temp = clk_plld_fbdv(plld);
	sysInfo->pllFbkDiv = get_cpr0_fbdv(temp);

	/* Calculate OPB clock divisor */
	mfcpr(clk_opbd, temp);
	temp = clk_opbd_opbdv(temp);
	sysInfo->pllOpbDiv = temp ? temp : 4;

	/* Calculate Peripheral clock divisor */
	mfcpr(clk_perd, temp);
	temp = clk_perd_perdv(temp);
	sysInfo->pllExtBusDiv = temp ? temp : 4;

	/* Calculate CPU clock divisor */
	mfcpr(clk_cpud, temp);
	temp = clk_cpud_cpudv(temp);
	cpudv = temp ? temp : 8;

	/* Calculate PLB2 clock divisor */
	mfcpr(clk_plb2d, temp);
	temp = clk_plb2d_plb2dv(temp);
        plb2dv = temp ? temp : 4;

	/* Calculate DDR2 clock divisor */
	mfcpr(clk_ddr2d, temp);
        temp = clk_ddr2d_ddr2dv(temp);
        ddr2dv = temp ? temp : 4;

	/* Calculate 'M' based on feedback source */
	mfcpr(clk_pllc, temp);
	temp = clk_pllc_sel(temp);
	if (temp == 0) {
		/* PLL internal feedback */
		m = sysInfo->pllFbkDiv;
	} else {
		/* PLL PerClk feedback */
		m = sysInfo->pllFwdDivA * sysInfo->pllFbkDiv * cpudv 
	...
	/* Now calculate the individual clocks */
	sysInfo->freqVCOMhz = (m * CONFIG_SYS_CLK_FREQ) + (m >> 1);
	sysInfo->freqProcessor = sysInfo->freqVCOMhz / sysInfo->pllFwdDivA / cpudv;
	sysInfo->freqPLB = sysInfo->freqVCOMhz / sysInfo->pllFwdDivA / cpudv / plb2dv / 2;
	sysInfo->freqOPB = sysInfo->freqPLB / sysInfo->pllOpbDiv;
	sysInfo->freqEBC = sysInfo->freqOPB / sysInfo->pllExtBusDiv;
	sysInfo->freqDDR = sysInfo->freqVCOMhz / sysInfo->pllFwdDivA / cpudv / ddr2dv / 2;
	sysInfo->freqUART = sysInfo->freqPLB;

	return;
}
#endif /* CONFIG_APM82181*/
