...
/* NOTE: This function is already duplicate in apm_ipp_relocate for OCM
 *       booting. If you update this function, do not forget to update
 *       in apm_ipp_relocate file as well.
 */
void get_sys_info (sys_info_t * sysInfo)
{
#ifdef CONFIG_ISS
	/* Now calculate the individual clocks */
	sysInfo->freqVCOMhz = 8000000;
	sysInfo->freqProcessor[CORE0_ID] = 8000000;
	sysInfo->freqProcessor[CORE1_ID] = 8000000;
	sysInfo->freqPLB = sysInfo->freqVCOMhz;
	sysInfo->freqOPB = 0;
	sysInfo->freqEBC = 1000000;
	sysInfo->freqUART = sysInfo->freqPLB/40;
#else
	int i, ret;
	u32 freq, div;
	...
	if (!ret) {
		sysInfo->freqVCOMhz = freq;
		for (i=CORE0_ID; i<CONFIG_MAX_CORES; i++) {
			ret = apm86xxx_get_cpu_divisor(i, &div);
			if (!ret) {
				sysInfo->freqProcessor[i] = (sysInfo->freqVCOMhz) / div;
			}
		}
	}

	/* SOC frequency PLL1 */
	ret = apm86xxx_get_freq(APM86xxx_SOC_PLL, &freq);
	if (!ret) {
		sysInfo->freqSOC = freq;
	}

	/* AXI divider */
	sysInfo->freqAXI = sysInfo->freqSOC / apm86xxx_get_divisor(APM86xxx_AXI);
	
	/* PLB Freq */
	ret = apm86xxx_get_plb_divisor(&div);
	if (!ret)
		sysInfo->freqPLB = sysInfo->freqVCOMhz / div;
	
	/* APB Freq */
	sysInfo->freqOPB = sysInfo->freqSOC / apm86xxx_get_divisor(APM86xxx_APB);
	
	/* Get AHB frequency */
	sysInfo->freqAHB = sysInfo->freqSOC / apm86xxx_get_divisor(APM86xxx_AHB);

	/* Get iAHB frequency */
	sysInfo->freqIAHB = sysInfo->freqSOC / apm86xxx_get_divisor(APM86xxx_IAHB);

	/* Uart speed fixed to SYS REF */
	sysInfo->freqUART = CONFIG_SYS_CLK_FREQ;

	/* EBC divider */
	sysInfo->freqEBC = sysInfo->freqSOC / apm86xxx_get_divisor(APM86xxx_EBUS);

	/* DDR frequency */
	ret = apm86xxx_get_freq(APM86xxx_DDR, &freq);
	if (!ret) {
		sysInfo->freqDDR = freq;
	}
#endif
}
...
int get_clocks (void)
{
	...
	gd->cpu_clk = sys_info.freqProcessor[mfspr(SPRN_PIR)];
#else
