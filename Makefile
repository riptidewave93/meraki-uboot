...
LIBS-$(CONFIG_IPROC_SLIC) += drivers/slic/libslic.o
LIBS-y += drivers/pwm/libpwmc.o
...
LIBS-y += mdk/libmdksys.o
LD_MDK_LIBS = -L./mdk-lib $(LDLIBS)
endif
...
# enable combining NAND_SPL/SPL/u-boot rules for brcm iproc
ifeq ($(VENDOR),broadcom)
ifeq ($(CONFIG_SPL),y)
ALL-y += $(obj)u-boot-iproc.bin
endif
endif
...
ifeq ($(VENDOR),broadcom)
ifeq ($(CONFIG_SPL),y)
$(obj)u-boot-iproc.bin: nand_spl $(obj)u-boot.bin $(obj)spl/u-boot-spl.bin 
		$(obj)tools/mkimage -A $(ARCH) -T firmware -C none \
		-O u-boot -a $(CONFIG_SYS_TEXT_BASE) -e $(CONFIG_SYS_TEXT_BASE) \
		-n $(shell sed -n -e 's/.*U_BOOT_VERSION//p' $(VERSION_FILE) | \
			sed -e 's/"[     ]*$$/ for $(BOARD) board"/') \
		-d  $(obj)u-boot.bin $(obj)u-boot.img
		$(OBJCOPY) ${OBJCFLAGS} -I binary \
			--pad-to=$(shell expr $$[$(CONFIG_SPL_MAX_SIZE)]) -O binary \
...
#
# Clean MDK target and packages:
#   make O=./build-output cleanmdk
#
ifeq ($(strip ${CFG_MDK}),1)
include $(TOPDIR)/mdk/ubootmdk.mk
endif # CFG_MDK end

ubootmdk: depend
	$(MAKE) CFG_MDK=1 mdkpkgs
	$(MAKE) CFG_MDK=1 mdklibs
	$(MAKE) CFG_MDK=1 BLDMDK=mdknolib all
#	$(MAKE) CFG_MDK=1 O=./build-output BLDMDK=mdknolib all

mdk-bcm53010:
	$(MAKE) CFG_MDK_DEBUG=1 CFG_LITTLE=1 MDK_CFLAGS="-DCDK_CONFIG_INCLUDE_FIELD_INFO=0 -DCDK_CONFIG_INCLUDE_BCM53010" ubootmdk
	
mdk-bcm53011:
	$(MAKE) CFG_MDK_DEBUG=1 CFG_LITTLE=1 MDK_CFLAGS="-DCDK_CONFIG_INCLUDE_FIELD_INFO=0 -DCDK_CONFIG_INCLUDE_BCM53011" ubootmdk

mdk-bcm53012:
	$(MAKE) CFG_MDK_DEBUG=1 CFG_LITTLE=1 MDK_CFLAGS="-DCDK_CONFIG_INCLUDE_FIELD_INFO=0 -DCDK_CONFIG_INCLUDE_BCM53012" ubootmdk

mdk-bcm53017:
	$(MAKE) CFG_MDK_DEBUG=1 CFG_LITTLE=1 MDK_CFLAGS="-DCDK_CONFIG_INCLUDE_FIELD_INFO=0 -DCDK_CONFIG_INCLUDE_BCM53017" ubootmdk

mdk-bcm53018:
	$(MAKE) CFG_MDK_DEBUG=1 CFG_LITTLE=1 MDK_CFLAGS="-DCDK_CONFIG_INCLUDE_FIELD_INFO=0 -DCDK_CONFIG_INCLUDE_BCM53018" ubootmdk

mdk-bcm53019:
	$(MAKE) CFG_MDK_DEBUG=1 CFG_LITTLE=1 MDK_CFLAGS="-DCDK_CONFIG_INCLUDE_FIELD_INFO=0 -DCDK_CONFIG_INCLUDE_BCM53019" ubootmdk

mdk-bcm53022:
	$(MAKE) CFG_MDK_DEBUG=1 CFG_LITTLE=1 BMD_PKG_OPTIONS="-c bcm53020_a0" CDK_PKG_OPTIONS="-c bcm53022" MDK_CFLAGS="-DCDK_CONFIG_INCLUDE_FIELD_INFO=0 -DCDK_CONFIG_INCLUDE_BCM53022" ubootmdk

mdk-bcm53025:
	$(MAKE) CFG_MDK_DEBUG=1 CFG_LITTLE=1 BMD_PKG_OPTIONS="-c bcm53020_a0" CDK_PKG_OPTIONS="-c bcm53025" MDK_CFLAGS="-DCDK_CONFIG_INCLUDE_FIELD_INFO=0 -DCDK_CONFIG_INCLUDE_BCM53025" ubootmdk
...
include/license.h: tools/bin2header COPYING LICENSE.TXT
	cat COPYING LICENSE.TXT | gzip -9 -c | ./tools/bin2header license_gzip > include/license.h
#########################################################################
...
#%_config::	unconfig
#	@$(MKCONFIG) -A $(@:_config=)
# The rule above forces a rebuild of all of uboot every make. Instead, include
# a Broadcom specific makefile snippet that properly handles dependencies.
include broadcom_boards.mk
...
#
# MDK target:
#   clean built target and packages
#
cleanmdk: 
