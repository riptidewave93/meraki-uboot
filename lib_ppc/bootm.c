...
#if defined(CONFIG_SYS_INIT_RAM_LOCK) || defined(CONFIG_MP)
#include <asm/cache.h>
#endif
...
void arch_lmb_reserve(struct lmb *lmb)
{
	...
	if (size < bootm_size) {
		...
		lmb_reserve(lmb, lmb_pa((void *) base), bootm_size - size);
	}
	...
	sp -= 1024;
	lmb_reserve(lmb, lmb_pa((void *) sp), 
		CONFIG_SYS_SDRAM_BASE + get_effective_memsize() - sp);
}
...
static int boot_body_linux(bootm_headers_t *images)
{
	...
	if (of_size) {
		...
		lmb_free(lmb, lmb_pa(*of_flat_tree),
		...
		lmb_reserve(lmb, lmb_pa(*of_flat_tree), of_size);
