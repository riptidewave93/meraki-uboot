...
#if defined(CONFIG_SAVE_FLASH_INFO)
extern int getfi(void);
#endif
...
{
#if defined(CONFIG_MP) && defined(CONFIG_SYS_MASTER_CPUID)
...
void board_init_f (ulong bootflag)
{
...
#if defined(CONFIG_VIDEO) && (defined(CONFIG_8xx) || defined(CONFIG_APM86XXX))
	 ...
	 * round down to next 4 kB limit
	 */
#if defined(CONFIG_PAL) || defined(CONFIG_HW_DDR)
	addr -= CONFIG_SYS_MONITOR_LEN;
#else
	addr -= len;
#endif
	addr &= ~(4096 - 1);
...
#if ((defined(CONFIG_CMD_NAND) && !defined(CONFIG_NAND_UFC)) || \
	(defined(CONFIG_NAND_UFC) && defined(CONFIG_SYS_NO_FLASH)))
	WATCHDOG_RESET ();
	puts ("NAND:  ");
...
#ifdef CONFIG_MERAKI
 {
	 extern int check_reset_boot (void);
	 check_reset_boot();
 }
#endif
...
#if defined(CONFIG_VIDEO) && (defined(CONFIG_8xx) || defined(CONFIG_APM86XXX))
	/* flush_dcache_range video display */
#ifdef CONFIG_MP
	if (CPUID == CONFIG_SYS_MASTER_CPUID)
#endif
		flush_dcache_range(gd->fb_base, gd->fb_base + VIDEO_COLS_MAX * VIDEO_ROWS_MAX * VIDEO_PIXEL_SIZE_MAX - 1);
#endif /* CONFIG_VIDEO  */
