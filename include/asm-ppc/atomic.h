...
static __inline__ int arch_spin_trylock(atomic_t *lock)
{
...
"1:	lwarx		%0,0,%2\n\
	...
	isync\n\
2:"	: "=&r" (tmp)
	: "r" (token), "r" (&lock->counter)
	: "cr0", "memory");

	return tmp;
}

static inline void atomic_csr_setbits(atomic_t *lock, u32 masks, void *csr)
{
	u32 val;
	
	/* Acquire the lock */
	while (lock != NULL) {
		if (arch_spin_trylock(lock) == 0)
			break;
	}
...
static inline void atomic_csr_clrbits(atomic_t *lock, u32 masks, void *csr)
{
	u32 val;
