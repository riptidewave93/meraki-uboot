...
#define CONFIG_SYS_MALLOC_LEN (4 << 20) /* 4MB for bzip2 */
...
#define CONFIG_SYS_NAND_U_BOOT_PARTITION 0x00170000	/* NAND reserved for U-Boot + environment + extra blocks */
...
#ifdef CONFIG_ENV_IS_IN_NAND
/*
 * The environment size is one NAND block (16K on MR24). We allocate four
...
#define CONFIG_SYS_HOSTNAME    mr24
#define CONFIG_SYS_PLATFORM    mr24
...
#define MERAKI_PANIC_PART CONFIG_SYS_NAND_U_BOOT_PARTITION
#define MERAKI_PANIC_SIZE 0x00010000

#define MERAKI_UBI_PART 0x00180000
#define MERAKI_UBI_SIZE 0x01e80000
...
#define CONFIG_AMCC_DEF_ENV_NAND_UPD                                    \
        "nload=${netloadmethod} 600000 ${serverpath}u-boot-nand.bin\0" \
	"nupdate=nand erase 0 " xstr(CONFIG_SYS_NAND_U_BOOT_PARTITION)" ;nand write 600000 0 " xstr(CONFIG_SYS_NAND_U_BOOT_SIZE) "\0" \
        "nupd=run nload nupdate\0"

#define CONFIG_MERAKI_ENV_SETTINGS					\
	"bootcmd=run meraki_boot\0"					\
	"meraki_bootargs=setenv bootargs root=/dev/ram console=ttyS0,${baudrate} ubi.mtd=ubi MERAKI_BOARD=mr24 mtdoops.mtddev=oops ${extra_bootargs}\0" \
	"meraki_bootfile=mr24.bin\0"					\
	...
	"meraki_loadaddr_ramdisk=e00000\0"				\
	...
	"meraki_ubi_update_nand=nand erase " xstr(MERAKI_UBI_PART) " " xstr(MERAKI_UBI_SIZE) " ; " \
        "nand write 600000 " xstr(MERAKI_UBI_PART) " ${filesize}\0"	\
...
#define	CONFIG_EXTRA_ENV_SETTINGS					\
	...
	"factory_kernel_addr_r=0x600000\0"				\
	"factory_fdt_addr_r=0xa00000\0"					\
...
#define CONFIG_LZMA 1		/* LZMA compression */
#define CONFIG_BZIP2 1		/* BZIP compreassion */
