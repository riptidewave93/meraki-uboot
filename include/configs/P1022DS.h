...
#define CONFIG_SYS_DDR_TIMING_1			0x5D5BD544
#define CONFIG_SYS_DDR_TIMING_2			0x0FA890CE
#define CONFIG_SYS_DDR_MODE_1			0x00061220
#define CONFIG_SYS_DDR_MODE_2			0x00000000
#define CONFIG_SYS_DDR_INTERVAL			0x0A28028A
 ...
 */
#define CONFIG_SYS_FLASH_BASE		0xee000000 /* start of FLASH 128M */
#ifdef CONFIG_PHYS_64BIT
...
#define CONFIG_FLASH_OR_PRELIM	(OR_AM_32MB | 0xff7)
...
#define CONFIG_SYS_NAND_BR_PRELIM  (BR_PHYS_ADDR(CONFIG_SYS_NAND_BASE_PHYS) \
			       ...
			       | BR_MS_FCM	       /* MSEL = FCM */ \
			       | BR_V)		       /* valid */
#define CONFIG_SYS_NAND_OR_PRELIM  ( OR_AM_512KB	       /* length 256K */ \
...
#define CONFIG_HOSTNAME		topaz
#define CONFIG_SYS_HOSTNAME	CONFIG_HOSTNAME
#define CONFIG_SYS_PLATFORM	CONFIG_SYS_PLATFORM
...
#define MERAKI_FIRMWARE_START	0
#define MERAKI_FIRMWARE_SIZE	0x00100000

#define MERAKI_FACTORY_START	0x00100000
#define MERAKI_FACTORY_SIZE	0x00100000

#define MERAKI_UBI_START	0x00200000
#define MERAKI_UBI_SIZE		0x07E00000

#define CONFIG_P1022_DEF_ENV_NAND_UPD						\
	"nload=${netloadmethod} ${loadaddr} ${serverpath}u-boot-nand.bin\0"	\
	"nupdate=nand erase " MK_STR(MERAKI_FIRMWARE_START) " " MK_STR(MERAKI_FIRMWARE_SIZE) " ;nand write ${loadaddr} " MK_STR(MERAKI_FIRMWARE_START) " " MK_STR(MERAKI_FIRMWARE_SIZE) " \0"				\
...
#define CONFIG_MERAKI_ENV_SETTINGS					\
	CONFIG_P1022_DEF_ENV_NAND_UPD					\
	"bootcmd=run first_loader\0"					\
	"netloadmethod=tftp\0"						\
	"accton_boot=setenv ipaddr 192.168.1.1; setenv serverip 192.168.1.120 ; tftp 0x2000000 Topaz-Accton-initramfs; tftp 0x3000000 Topaz-Accton-p1022ds.dtb; bootm 0x2000000 - 0x3000000\0" \
	"first_loader=if meraki rst_sw; then run accton_boot; fi; run second_loader \0" \
	"second_loader=nand read 0xc00000 0x100000 0x1000; if meraki checkpart 0xc00000; then run meraki_boot; else run accton_boot; fi\0" \
	"meraki_bootargs=setenv bootargs root=/dev/ram console=ttyS0,${baudrate} ubi.mtd=ubi MERAKI_BOARD=topaz mtdoops.mtddev=oops mem=255M ${extra_bootargs}\0" \
	"meraki_bootfile=topaz.bin\0"					\
	...
	"run meraki_load2 meraki_bootlinux\0"				\
	"meraki_checkpart=meraki checkpart ${meraki_loadaddr}\0"	\
	"meraki_netboot=run meraki_load_net meraki_bootargs meraki_bootlinux\0" \
        "meraki_loadaddr=c00000\0"					\
	"meraki_loadaddr_kernel=c10000\0"				\
	"meraki_loadaddr_fdt=c00400\0"					\
	"meraki_loadaddr_ramdisk=1000000\0"				\
	...
	"meraki_ubifile=topaz-ubi.bin\0"					\
	"meraki_ubi_loadfile=${netloadmethod} 600000 ${serverpath}${meraki_ubifile}\0" \
	"meraki_ubi_update_nand=nand erase " MK_STR(MERAKI_UBI_START) " " MK_STR(MERAKI_UBI_SIZE) " ; " \
        "nand write 1000000 " MK_STR(MERAKI_UBI_PART) " ${filesize}\0"	\
	...
	"mtdparts=mtdparts=nand0:" MK_STR(MERAKI_FIRMWARE_SIZE) "@0(firmware)," \
	MK_STR(MERAKI_FACTORY_SIZE) "@" MK_STR(MERAKI_FACTORY_SIZE)"(factory)," \
	 MK_STR(MERAKI_UBI_SIZE) "@" MK_STR(MERAKI_UBI_START)"(ubi)\0"		\

#define	CONFIG_EXTRA_ENV_SETTINGS					\
	"perf_mode=stable\0"						\
	"memctl_intlv_ctl=2\0"						\
	"netdev=eth0\0"							\
	"ethaddr=00:01:73:01:23:42\0"					\
	"consoledev=ttyS0\0"						\
	CONFIG_MERAKI_ENV_SETTINGS					\
	"ipaddr=10.3.15.233\0"						\
	"serverip=10.3.15.231\0"						\
	"serverpath=10.3.15.231:robg/\0"						\
	"monitor=0\0"							\
