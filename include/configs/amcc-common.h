...
#define CONFIG_SYS_SDRAM_BASE_1GB	0x40000000	/* _must_ be 0		*/
#define CONFIG_SYS_MONITOR_BASE		TEXT_BASE	/* Start of U-Boot	*/
#define CONFIG_SYS_MONITOR_LEN		(0xFFFFFFFF - CONFIG_SYS_MONITOR_BASE + 1)
#define CONFIG_SYS_MALLOC_LEN		(1 << 22)	/* Reserved for malloc	*/
...
#define CONFIG_AMCC_DEF_ENV_POWERPC					\
	...
	"net_nfs=tftp ${kernel_addr_r} ${user_dir}/${bootfile}; "	\
		"tftp ${fdt_addr_r} ${user_dir}/${fdt_file}; "		\
		"run nfsargs addip addtty addmisc; "			\
		"bootm ${kernel_addr_r} - ${fdt_addr_r}\0"		\
	"net_self_load=tftp ${kernel_addr_r} ${user_dir}/${bootfile}; "	\
		"tftp ${fdt_addr_r} ${user_dir}/${fdt_file}; "		\
		"tftp ${ramdisk_addr_r} ${user_dir}/${ramdisk_file}\0"	\
	"net_self=run net_self_load; "					\
		"run ramargs addip addtty addmisc; "			\
		"bootm ${kernel_addr_r} ${ramdisk_addr_r} ${fdt_addr_r}\0" \
	"fdt_file=" xstr(CONFIG_HOSTNAME) ".dtb\0"
...
#define CONFIG_AMCC_DEF_ENV_PPC_OLD					\
	...
	"flash_nfs_old=run nfsargs addip addtty addmisc; "		\
		"bootm ${kernel_addr}\0"				\
	"net_nfs_old=tftp ${kernel_addr_r} ${user_dir}/${bootfile}; "	\
		"run nfsargs addip addtty addmisc; "			\
		"bootm ${kernel_addr_r}\0"

#define CONFIG_AMCC_DEF_ENV_NOR_UPD					\
	"u-boot=u-boot.bin\0"						\
	"load=tftp 200000 ${user_dir}/${u-boot}\0"			\
...
#define CONFIG_AMCC_DEF_ENV_NAND_UPD					\
	"u-boot-nand=u-boot-nand.bin\0"	\
	"ethaddr=00:11:3a:8a:5a:6c\0" \
	"nload=tftp 200000 ${user_dir}/${u-boot-nand}\0"		\
	"nupdate=nand erase 0 C0000; nand write 200000 0 C0000; "	\
		"setenv filesize\0"				\
