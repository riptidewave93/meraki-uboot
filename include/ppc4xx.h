...
#ifdef CONFIG_APM86XXX
#define EBC_DCR_BASE	(CONFIG_SYS_EBC_BASE + 0x48)
#define EBC0_CFGADDR	(EBC_DCR_BASE + 0x0)
#define EBC0_CFGDATA	(EBC_DCR_BASE + 0x4)
#else
...
#ifdef CONFIG_APM86XXX
#define mtebc(reg, d)	\
  do { *(volatile unsigned int *)(EBC0_CFGADDR) = reg;*(volatile unsigned int *)(EBC0_CFGDATA) = d; } while (0)
#define mfebc(reg, d)	\
  do { *(volatile unsigned int *)(EBC0_CFGADDR) = reg;d = *(volatile unsigned int *)(EBC0_CFGDATA); } while (0)

#define mtsdram(reg, d)	\
  do { *(volatile unsigned int *)(CONFIG_SYS_DDRC_BASE + reg) = d; } while (0)
#define mfsdram(reg, d)	\
  do { d = *(volatile unsigned int *)(CONFIG_SYS_DDRC_BASE + reg); } while (0)
#else
#define mtebc(reg, d)	\
...
#ifdef CONFIG_APM86XXX
	unsigned long freqSOC;
	unsigned long freqAXI;
	unsigned long freqAHB;
	unsigned long freqIAHB;
#endif
	unsigned long freqTmrClk;
	unsigned long freqUART;
#ifdef CONFIG_APM86XXX
	unsigned long freqProcessor[CONFIG_MAX_CORES];
#else
