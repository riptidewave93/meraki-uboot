...
static void ppc4xx_setup_utl(u32 port)
{
	...
	out_be32(utl_base + PEUTL_RCIRQEN, 0x00f00000);
#if defined(CONFIG_APM82181)
	out_be32(utl_base + PEUTL_PCTL, 0x80841066);   // Reset in/outbound tags read allocation to apply new values
	mdelay(50);
#endif
...
#if defined(CONFIG_APM82181) 
		SDR_WRITE(PESDR0_PHY_CTL_RST,0x10000000); /* EAS */
		mdelay(50);
		SDR_WRITE(PESDR0_PHY_CTL_RST,0x30000000); /* EAS */
#else
...
int __ppc4xx_init_pcie_port_hw(int port, int rootport)
{
	...
	while (!(SDR_READ(SDRN_PESDR_PHYSTA(port)) & 0x00001000)) {
		...
		timeout++;
		if (timeout == 10) { 
			printf("PCIE%d: PHY reset failed\n", port);
			break;	
		}
	}
...
static inline u64 ppc4xx_get_cfgaddr(int port)
{
	...
	if (port == 0)
		...
		return 0x0000000d20000000ULL;
#endif
	return 0; /* Just to satisfy compliler */
}
...
#if defined(CONFIG_APM82181)
//Set PCIE 256B Max Payload Size
	printf("Configure Max Payload 256B \n");
	/*
	PECFG_ECDEVCAPPA[PSIZE] = 0b001.
	PECFG_ECDEVCTL[RSIZE] = 0b001.
	PECFG_ECDEVCTL[MPSZ] = 0b001.
	*/
	out_le32(mbase + PECFG_ECDEVCAPPA, (in_le32(PECFG_ECDEVCAPPA)&0xfffffff8)|0x01);
	out_le32(mbase + PECFG_ECDEVCTL, (in_le32(PECFG_ECDEVCTL)&0xffff8f1f)|0x1020);
#endif
...
int ppc4xx_setup_pcie_endpoint(struct pci_controller *hose, int port)
{
	...
	case 0:
		...
		hose->cfg_data = (u8 *)CONFIG_SYS_PCIE0_CFGBASE;
		break;
#if defined(CONFIG_SYS_PCIE1_XCFGBASE)
	case 1:
		mbase = (u32 *)CONFIG_SYS_PCIE1_XCFGBASE;
		hose->cfg_data = (u8 *)CONFIG_SYS_PCIE1_CFGBASE;
		break;
	...
	out_le32(mbase + PECFG_PIMEN, 0x1);
#if defined(CONFIG_APM82181)
	//Seting PCIE 256B May Payload Size
	printf("Configure Max Payload 256B \n");
	/*
	PECFG_ECDEVCAPPA[PSIZE] = 0b001.
	PECFG_ECDEVCTL[RSIZE] = 0b001.
